...
{ // function
    ...
    {         // for loop
        ...
        if (xSemaphoreTake(xLCDDataMutex, 250 / portTICK_PERIOD_MS) == pdPASS) // 给温度数组的最后一个数值写入数据
        {
            BT_TEMP_LCD = BT_TEMP;
            ET_TEMP_LCD = ET_TEMP;
            ror_LCD = ror;
...
{ // read analog port 1 and adjust OT1 output
    ...
    if (pid_status == false)
    {
        ...
        {                                                            // did it change?
            ...
            if (xSemaphoreTake(xThermoDataMutex, 250 / portTICK_PERIOD_MS) == pdPASS) // 给温度数组的最后一个数值写入数据
            {
                levelOT1 = reading;
                pwm_heat.write(map(levelOT1, 0, 100, PWM_HEAT_MIN, PWM_HEAT_MAX));
                xSemaphoreGive(xThermoDataMutex); // end of lock mutex
...
#endif // end of __TASK_READ_TEMP_H__
