...
#else
       // BT_TEMP = pid_parm.BT_tempfix + (((Voltage / 1000 * Rref) / ((3.3 * 1000) - Voltage / 1000) - R0) / (R0 * 0.0036));
        BT_TEMP = pid_parm.BT_tempfix + (((Voltage / 1000 * Rref) / ((3.3 * 1000) - Voltage / 1000) - R0) / (R0 * 0.0038));
#endif
        ET_TEMP = 0.0;
        // cal RoR
        ftimes = millis();
        ...
        if (xSemaphoreTake(xThermoDataMutex, timeOUT) == pdPASS) // 给温度数组的最后一个数值写入数据
        {
            ...
            sprintf(temp_data_buffer_ble, "#0.00,%4.2f,%d,%d;\r\n", BT_TEMP, levelOT1, levelIO3);
            ...
            for (int loop = 0; loop < 4; loop++)
            {
                ...
                case 0:
                    ...
                    while (!tuner.isFinished()) // 开始自动整定循环
                    {
                        ...
                        Serial.printf("PID Auto Tuneing...OUTPUT:%4.2f ,IO1:%d ,BT_temp:%4.2f\n", pid_tune_output, levelOT1, BT_TEMP);
#endif
                ...
                case 1:
                    ...
                    tuner.startTuningLoop(pid_parm.pid_CT * uS_TO_S_FACTOR);
                    tuner.setOutputRange((PID_STAGE_2_MIN_OUT * 255 / 100), (PID_STAGE_2_MAX_OUT * 255 / 100));
                    tuner.setTuningCycles(PID_TUNE_CYCLE);
