...
void setup()
{
    ...
    pwm_heat.write(FAN_OUT_PIN, 600, frequency, resolution); // 此处要根据100g豆子初始化吹动的风力来确定。数值为0-1024
    pwm_heat.resume();
    pwm_heat.printDebug();
...
void Task_PID_autotune(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (xResult == pdTRUE)
        {
            ...
            for (int loop = 0; loop < 3; i++)
            {
                if (loop == 0)
                {
                    PID_TUNE_SV == PID_TUNE_SV_1;
                    tuner.setTargetInputValue(PID_TUNE_SV);
                    pwm_heat.write(FAN_OUT_PIN, 615, frequency, resolution); // 设定风力60
                    pwm_heat.write(HEAT_OUT_PIN, 0, frequency, resolution);  // 设定火力关闭
                    vTaskDelay(1000);
                    while (!tuner.isFinished()) // 开始自动整定循环
                    {
                        ...
                        if (xSemaphoreTake(xThermoDataMutex, xIntervel) == pdPASS) // 给温度数组的最后一个数值写入数据
                        {
                            ...
                            pwm_heat.write(HEAT_OUT_PIN, map(pid_tune_output, 0, 255, 230, 950), frequency, resolution); // 输出新火力pwr到SSRÍ
                            xSemaphoreGive(xThermoDataMutex);                                                            // end of lock mutex
                        }
