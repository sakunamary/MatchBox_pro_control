...
void PIDAutotuner::startTuningLoop() {
    ...
    max_cal = -1000000; // Max input
    min_cal = 1000000; // Min input
...
double PIDAutotuner::tunePID(double input) {
    ...
    max_cal = max(max_cal, input);
    min_cal = min(min_cal, input);
    ...
    if (!output && input < targetInputValue) {
        ...
        double ku = (4.0 * ((maxOutput - minOutput) / 2.0)) / (M_PI * (max_cal - min_cal) / 2.0);
